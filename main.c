...
#pragma config(Motor,  port4,           scorer,        tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port5,           intake,        tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port6,           rightBackMotor, tmotorVex393_MC29, openLoop)
...
void arcadeDrive(float drive, float turn) {
	motor[leftFrontMotor]  = drive + turn;
	motor[leftBackMotor]  = drive + turn;
...
void driveDist(float feet) {
	...
	while ((getLeftDistance() - leftStart) < feet || (getRightDistance() - rightStart) < feet) {
		...
		arcadeDrive(127.0 * speedPct, 0);
	}

	//float startDist = getDistance();
	//startMotionProfile(AUTON_MAX_VEL, AUTON_MAX_ACCEL);

	//while (getDistance() - startDist < feet) {
	//	float speed = getProfiled(feet) * 127;
	//	arcadeDrive(speed, 0);
	//}
...
task main() {
	...
	while (true) {
		...
		lastIntakeToggleButton = vexRT[Btn8U];
